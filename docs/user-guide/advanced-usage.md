...
the workflow fetches it from the version tag the API project's
Dependabot pins.
...
#     from this repository on the base branch, not taken from the PR).
...
* **`script-url-template`** — points to the
  ...
  the script is meant to be evolved in lockstep with API consumers and
  should always be fetched from the same branch.
...
!!! Warning "Security"
    ...
    migration script is fetched from an upstream tag.  For details, see
